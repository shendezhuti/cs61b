...
public class BinaryTree implements Dictionary {
  ...
  public Entry remove(Object key) {
    ...
    if(node==null){
      ...
      if(node.leftChild==null&&node.rightChild==null){//如果是叶子节点
        if(node==root){
      ...
      }else if(node.leftChild==null||node.rightChild==null){//内部结点，并且有一个子节点
          if(node.leftChild==null){
          ...
          if(node.rightChild.leftChild==null){//右子节点是比要删除结点中最小的值
            node.entry=node.rightChild.entry;//他这里改变了entry
